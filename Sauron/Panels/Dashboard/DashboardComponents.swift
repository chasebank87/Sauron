    ...
    var body: some View {
        HStack(spacing: 10) {
            Image(systemName: systemImage)
                ...
                .foregroundStyle(focused ? SauronTheme.irisSolid : SauronTheme.textTertiary)
        ...
        .padding(.vertical, 10)
        .background(SauronTheme.surfaceElevated, in: RoundedRectangle(cornerRadius: SauronTheme.radiusControl, style: .continuous))
        .overlay(
            RoundedRectangle(cornerRadius: SauronTheme.radiusControl, style: .continuous)
                .strokeBorder(
                    focused ? SauronTheme.irisSolid.opacity(0.55) : SauronTheme.hairline,
                    lineWidth: 1
                )
        )
    }
...
struct DashboardTextField: View {
    ...
    var body: some View {
        TextField(placeholder, text: $text)
            ...
            .padding(.vertical, 10)
            .focused($focused)
            .background(SauronTheme.surfaceSunken, in: RoundedRectangle(cornerRadius: SauronTheme.radiusControl, style: .continuous))
            .overlay(
                RoundedRectangle(cornerRadius: SauronTheme.radiusControl, style: .continuous)
                    .strokeBorder(
                        focused ? SauronTheme.irisSolid.opacity(0.45) : SauronTheme.hairline,
                        lineWidth: 1
                    )
            )
    }
            ...
            HStack(alignment: .firstTextBaseline, spacing: 8) {
                ...
                if let delta {
                    Text(delta)
                        ...
                        .foregroundStyle(deltaPositive ? SauronTheme.mint : SauronTheme.red)
                        .padding(.horizontal, 6)
                        .padding(.vertical, 2)
                        .background(
                            (deltaPositive ? SauronTheme.mint : SauronTheme.red).opacity(0.14),
                            in: Capsule()
                        )
                }
            }
...
struct MeetingRowCard: View {
    ...
    var body: some View {
        Button(action: {
            ...
            HStack(spacing: 12) {
                if isSelecting {
                    Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
                        .font(.title3)
                        .foregroundStyle(isSelected ? SauronTheme.irisSolid : SauronTheme.textTertiary)
                        .frame(width: 28)
                }

                ZStack {
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .fill(SauronTheme.irisGradient.opacity(0.22))
            ...
            .padding(12)
            .background(
                (isSelected || hovered) ? SauronTheme.surfaceElevated : SauronTheme.surface,
            ...
            .overlay(
                RoundedRectangle(cornerRadius: SauronTheme.radiusCard, style: .continuous)
                    .strokeBorder(
                        isSelected ? SauronTheme.irisSolid.opacity(0.55) : SauronTheme.hairline,
                        lineWidth: 1
                    )
            )
        }
...
struct AskRowCard: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            Button(action: onToggle) {
                Image(systemName: item.status == .done ? "checkmark.circle.fill" : "circle")
                    .font(.title3)
                    .foregroundStyle(item.status == .done ? SauronTheme.mint : SauronTheme.textTertiary)
            }
            .buttonStyle(.plain)
                ...
                HStack(spacing: 8) {
                    ...
                    if isOverdue {
                        Text(agingLabel)
                            ...
                            .padding(.vertical, 2)
                            .background(SauronTheme.red.opacity(0.14), in: Capsule())
                    }
                }
            }
            Spacer(minLength: 0)
        ...
        .background(
            selected ? SauronTheme.surfaceElevated : SauronTheme.surface,
            in: RoundedRectangle(cornerRadius: SauronTheme.radiusCard, style: .continuous)
        )
        .overlay(alignment: .leading) {
            if isOverdue && item.status == .open {
                RoundedRectangle(cornerRadius: 2)
                    .fill(SauronTheme.red)
                    .frame(width: 2)
                    .padding(.vertical, 8)
            }
        }
