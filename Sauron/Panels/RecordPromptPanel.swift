        ...
        VStack(spacing: 0) {
            VStack(spacing: 6) {
                ZStack {
                    if alertEnabled {
                        Circle()
                            .stroke(SauronTheme.accent.opacity(0.35), lineWidth: 2)
            ...
            Label("Wear headphones — without them the meeting's speaker audio leaks into your mic", systemImage: "exclamationmark.triangle.fill")
                .font(.caption)
        ...
        Button(action: action) {
            VStack(spacing: 0) {
                Image(systemName: media.systemImage)
                    .font(.system(size: 22))
                    .foregroundStyle(isSelected ? SauronTheme.accent : Color.secondary)
                Text(media.title)
            ...
            .background {
                RoundedRectangle(cornerRadius: cardRadius, style: .continuous)
                    .fill(isSelected
                          ? SauronTheme.accent.opacity(0.10)
                          : SauronTheme.fillQuiet(for: colorScheme))
                RoundedRectangle(cornerRadius: cardRadius, style: .continuous)
                    .strokeBorder(
                        isSelected ? SauronTheme.accent : SauronTheme.hairline(for: colorScheme),
        ...
        ZStack {
            if showsAmbient {
                ...
                RoundedRectangle(cornerRadius: SauronTheme.cardRadius, style: .continuous)
                    .fill(SauronTheme.panelFallback(for: colorScheme))
            }
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                ScrollViewReader { proxy in
                    ...
                    .onChange(of: appState.liveTranscriptFocusID) { _, id in
                    ...
                    .onChange(of: appState.liveSegments.last?.text) { _, _ in
                        if let id = appState.liveTranscriptFocusID ?? appState.liveSegments.last?.id {
                            withAnimation(.easeOut(duration: 0.2)) {
                                proxy.scrollTo(id, anchor: .bottom)
                            }
                        }
                ...
                GlassEffectContainer(spacing: 8) {
                    HStack(spacing: 10) {
                        Button {
                            ...
                            Label(
                                appState.isMicMuted ? "Unmute me" : "Mute me",
                                systemImage: appState.isMicMuted ? "mic.slash.fill" : "mic.fill"
                            )
                        }
                        .observerGlassButton()
                        .tint(appState.isMicMuted ? SauronTheme.amber : nil)
                        .disabled(appState.status != .recording)
                        .accessibilityLabel(appState.isMicMuted ? "Unmute me" : "Mute me")
                        Text(modeCaption)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                        Spacer()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            ReactiveWaveformLine(
                level: Double(monitor.displayLevel(includingMic: !micMuted)),
                isAlert: monitor.anySilent
            )
            .frame(height: 48)
            .opacity(micMuted ? 0.55 : 1)
            .accessibilityLabel("Combined audio level")
            .accessibilityValue(monitor.anySilent
                                ? "Silent"
                                : "\(Int(monitor.displayLevel(includingMic: !micMuted) * 100)) percent")
        }
        .padding(12)
        .background {
    ...
    var body: some View {
        HStack(alignment: .bottom, spacing: 0) {
            ...
            VStack(alignment: isYou ? .trailing : .leading, spacing: 4) {
                Text(displayName)
                    .font(.caption2.weight(.semibold))
                    .foregroundStyle(isYou ? SauronTheme.accent : Color.secondary)
                ...
                Text(segment.text)
                    ...
                    .background {
                        RoundedRectangle(cornerRadius: 14, style: .continuous)
                            .fill(isYou
                                  ? SauronTheme.accent.opacity(colorScheme == .dark ? 0.28 : 0.18)
                    ...
                    .overlay {
                        RoundedRectangle(cornerRadius: 14, style: .continuous)
                            .strokeBorder(
                                isYou
                                    ? SauronTheme.accent.opacity(0.35)
