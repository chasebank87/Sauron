...
enum HermesMCPInstaller {
    ...
    static func installSauronMemory(endpoint: URL, token: String) throws -> InstallResult {
        if isDesktopAppInstalled() {
            ...
            return InstallResult(
                message: "Opened Hermes desktop — confirm “sauron” MCP install there. Keep Sauron running afterward.",
                method: .desktop,
    ...
    static func installDeepLink(endpoint: URL, token: String) -> URL? {
        ...
        components.queryItems = [
            URLQueryItem(name: "name", value: "sauron"),
            URLQueryItem(name: "config", value: encoded)
        ]
        return components.url
    }

    /// Writes `mcp_servers.sauron` into `~/.hermes/config.yaml` and stores the bearer token in `~/.hermes/.env`.
    static func installViaCLI(endpoint: URL, token: String) throws -> InstallResult {
        ...
        endpoint = sys.argv[1]
        token = sys.argv[2]
        existing = _get_mcp_servers().get("sauron") or {}
        already = bool(existing.get("url") == endpoint)
        headers = _save_bearer_auth_token("sauron", token)
        ok = _save_mcp_server("sauron", {
            "url": endpoint,
            "headers": headers,
            "enabled": True,
        })
        if not ok:
            ...
            raise SystemExit(2)
        print(json.dumps({"ok": True, "already": already, "env_key": "MCP_SAURON_API_KEY"}))
        """
        ...
        let message = payload.already == true
            ? "Updated Hermes CLI MCP “sauron” → \(host):\(port) — token refreshed in ~/.hermes/.env. Run /reload-mcp or restart Hermes."
            : "Installed Sauron Memory into Hermes CLI as MCP “sauron”. Restart Hermes or run /reload-mcp, then keep Sauron running."
