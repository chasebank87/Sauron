...
enum LLMProviderKind: String, CaseIterable, Identifiable, Sendable {
    ...
    /// Backends that can host the local meeting-memory embedding index.
    var supportsEmbeddings: Bool {
        switch self {
        case .ollama, .lmStudio, .openRouter: true
        case .hermes, .openClaw: false
        }
    }

    /// Providers offered as the separate embeddings backend for Hermes/OpenClaw.
    static var embeddingBackends: [LLMProviderKind] {
        allCases.filter(\.supportsEmbeddings)
    }
...
struct LLMClient: Sendable {
    ...
    private func applyAuth(to request: inout URLRequest) {
        ...
        case .openRouter:
            ...
            request.setValue("Sauron", forHTTPHeaderField: "X-Title")
