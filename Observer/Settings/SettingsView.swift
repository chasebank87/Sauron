...
struct GeneralSettingsView: View {
    ...
    private func chooseRecordingsFolder() {
        ...
        panel.prompt = "Select"
        panel.message = "Choose where Observer saves meeting video and audio."
...
private struct CalendarSubscriptionsSection: View {
    @Environment(AppState.self) private var appState
    @State private var calendars: [ObserverCalendarInfo] = []
...
struct ProviderSettingsView: View {
        ...
        Form {
            Section("Provider") {
                ...
                case .hermes:
                    ...
                    Button(
                        HermesMCPInstaller.isDesktopAppInstalled()
                            ? "Add Observer Memory to Hermes"
                            : "Install Observer Memory into Hermes CLI"
                    ) {
                        installObserverMemoryIntoHermes()
                    }
                    ...
                    Text(
                        settings.shouldRunMemoryMCPServer
                            ? (HermesMCPInstaller.isDesktopAppInstalled()
                                ? "Opens the Hermes desktop app with a confirm dialog to install Observer’s localhost MCP."
                                : "Hermes desktop not detected — writes mcp_servers.observer into ~/.hermes/config.yaml and the token into ~/.hermes/.env. Restart Hermes or run /reload-mcp afterward.")
                ...
                TextField(
                    ...
                    prompt: Text(SettingsStore.defaultEmbeddingModel(for: settings.providerKind))
                )
            }
            Section {
    ...
    private func installObserverMemoryIntoHermes() {
        appState.syncMemoryMCPServer()
        let token = KeychainStore.mcpServerToken
        do {
            let result = try HermesMCPInstaller.installObserverMemory(
        ...
        Form {
            Section("Live Assist") {
                ...
                if settings.providerKind.providesBuiltInWebResearch {
                    Text("During a meeting, Observer extracts claims and questions and asks \(settings.providerKind.displayName) to research them with its built-in tools. Tavily is not used with this provider.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                } else {
                    Text("During a meeting, Observer extracts short claims and questions from the transcript. With a Tavily key, it can fact-check and research on the web.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Stepper(
