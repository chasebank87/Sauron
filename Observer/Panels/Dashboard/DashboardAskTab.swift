...
struct DashboardAskTab: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            if !appState.settings.memoryEnabled {
                Text("Memory is off — answers won’t cite past meetings.")
                    .font(.system(size: 11))
                    .foregroundStyle(ObserverTheme.amber)
                    .padding(.horizontal, 16)
                    .padding(.bottom, 8)
            }
                    ...
                    threadRow(thread)
                        ...
                        .listRowBackground(
                            (!isSelectingThreads && selectedID == thread.id)
                                ? ObserverTheme.surfaceElevated
                                : ObserverTheme.surface.opacity(0.001)
                        )
                }
            }
            .listStyle(.sidebar)
        ...
        return HStack(spacing: 8) {
            if isSelectingThreads {
                Image(systemName: checked ? "checkmark.circle.fill" : "circle")
                    .foregroundStyle(checked ? ObserverTheme.irisSolid : ObserverTheme.textTertiary)
            }
    ...
    private func messageBlock(_ message: DashboardChatMessage) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(message.role == "user" ? "You" : "Observer")
                .font(.system(size: 11, weight: .semibold))
                .foregroundStyle(ObserverTheme.textTertiary)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ForEach(0..<3, id: \.self) { _ in
                        RoundedRectangle(cornerRadius: 4)
                            .fill(ObserverTheme.surfaceElevated)
                            .frame(height: 10)
                            .overlay(ObserverTheme.irisGradient.opacity(0.25))
                    }
                }
            } else {
                Text(streamBuffer)
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(alignment: .bottom, spacing: 10) {
                ...
                Button(action: { Task { await send() } }) {
                    Image(systemName: "arrow.up")
                        ...
                        .background(ObserverTheme.irisGradient, in: Circle())
                }
                .buttonStyle(.plain)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .strokeBorder(ObserverTheme.irisSolid.opacity(isSending ? 0.55 : 0.35), lineWidth: 1)
            )
        }
        .padding(16)
        .background(ObserverTheme.canvas.opacity(0.85))
    }

    private func scopeChip(_ title: String, days: Int) -> some View {
        Button(title) { scopeDays = days }
            .font(.system(size: 11, weight: .medium))
            .foregroundStyle(scopeDays == days ? ObserverTheme.textPrimary : ObserverTheme.textSecondary)
            .padding(.horizontal, 10)
            .padding(.vertical, 6)
            .background(
                scopeDays == days ? ObserverTheme.irisSolid.opacity(0.25) : ObserverTheme.surface,
                in: Capsule()
            )
            .buttonStyle(.plain)
    }
    ...
    private func send() async {
        ...
        do {
            ...
            if injectMemory {
                system = """
                You are Observer's meeting memory assistant. Answer using the provided past-meeting excerpts when available.
                ...
                system = """
                You are Observer's meeting memory assistant.
