...
struct DashboardMemoryTab: View {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    VStack(alignment: .leading, spacing: 12) {
                        ...
                        DashboardTextField(
                            placeholder: SettingsStore.defaultEmbeddingModel(for: settings.providerKind),
    ...
    private var indexHealth: some View {
        ...
        return HStack(spacing: 16) {
            ZStack {
                Circle()
                    .stroke(ObserverTheme.surfaceElevated, lineWidth: 10)
                Circle()
                    .trim(from: 0, to: chunks == 0 ? 0.05 : min(1, Double(chunks) / 500))
                    .stroke(ObserverTheme.irisGradient, style: StrokeStyle(lineWidth: 10, lineCap: .round))
    ...
    private var privacyCard: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            Text("The memory index is stored only on this Mac. Embedding requests send short text chunks to your selected provider to compute vectors; Observer does not host your meetings in the cloud. Retrieved snippets may be sent to your chat model when summarizing or asking. Hermes/OpenClaw use MCP tools instead of prompt injection.")
                .font(.system(size: 12))
                .foregroundStyle(ObserverTheme.textSecondary)
        }
        .padding(16)
    ...
    private var mcpServerCard: some View {
        ...
        return VStack(alignment: .leading, spacing: 12) {
            ...
            Text(server.state.statusLabel)
                ...
                .foregroundStyle(server.state.isListening ? ObserverTheme.irisSolid : ObserverTheme.textSecondary)
            ...
            HStack(spacing: 8) {
                Text("Port")
                    .foregroundStyle(ObserverTheme.textSecondary)
                TextField("8787", value: $settings.mcpServerPort, format: .number)
            ...
            Text("Loopback only. Hermes and OpenClaw must be configured to call this MCP URL while Observer is running. Other providers still receive memory excerpts in the prompt.")
                .font(.system(size: 11))
                .foregroundStyle(ObserverTheme.textTertiary)
        }
        .padding(16)
    ...
    private func snippetBlock(_ text: String) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(text)
                ...
                .background(ObserverTheme.surfaceElevated, in: RoundedRectangle(cornerRadius: 8, style: .continuous))
            Button("Copy snippet") {
                NSPasteboard.general.clearContents()
    ...
    private func testEmbeddings() async {
        ...
        do {
            ...
            let vectors = try await client.embed(model: model, texts: ["Observer memory probe"])
            status = "OK — \(vectors.first?.count ?? 0)-dim vector from \(model)"
        } catch {
