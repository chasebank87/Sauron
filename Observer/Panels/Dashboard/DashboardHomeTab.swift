...
struct DashboardHomeTab: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .strokeBorder(ObserverTheme.irisSolid.opacity(0.35), lineWidth: 1)
            )
            ...
            HStack(spacing: 8) {
                ForEach(suggestions, id: \.self) { chip in
                    ...
                    .padding(.vertical, 6)
                    .background(ObserverTheme.surface, in: Capsule())
    ...
    private func upNextCard() -> some View {
        TimelineView(.periodic(from: .now, by: 30)) { context in
                    ...
                    HStack(spacing: 8) {
                        ...
                        if next.isInProgress(at: now) {
                            Text("Now")
                                ...
                                .padding(.vertical, 4)
                                .background(ObserverTheme.irisSolid.opacity(0.15), in: Capsule())
                        }
                    }
                } else {
            ...
            .background(
                LinearGradient(
                    colors: [ObserverTheme.irisStart.opacity(0.28), ObserverTheme.irisEnd.opacity(0.12), ObserverTheme.surface],
                    startPoint: .topLeading,
                    endPoint: .bottomTrailing
                ),
                in: RoundedRectangle(cornerRadius: ObserverTheme.radiusPanel, style: .continuous)
            )
            .overlay(
                RoundedRectangle(cornerRadius: ObserverTheme.radiusPanel, style: .continuous)
                    .strokeBorder(ObserverTheme.irisSolid.opacity(0.25), lineWidth: 1)
            )
        }
    }
